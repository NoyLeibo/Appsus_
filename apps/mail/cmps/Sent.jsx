...
export function Sent() {
  ...
  useEffect(() => {
    mailService
      ...
      .then((mails) => {
        mails = mails.filter(
          (mail) =>
            mail.from === "user@appsus.com" && mail.sentAt && !mail.removedAt
        );
  ...
  function onRemoveMail(mail) {
    ...
    showSuccessMsg("The Email was deleted");
    setRender(!render);
  }
          ...
          <tbody>
            {mails.map((mail) => (
              <tr key={mail.id} className={dynClassRead}>
                ...
                <td onClick={() => isOpenMail(mail)}>{mail.sendTo}</td>
                <td onClick={() => isOpenMail(mail)}>
                  <span className="subject">
                    {mail.subject}
                    <LongTxt txt={mail.body} length={mail.body.length} />
                  </span>
                </td>
                <td className="deliveryDate">
                  {new Date(mail.sentAt).getDate() +
                    " / " +
                    new Date(mail.sentAt).getMonth()}
                </td>
                <td>
                  <button
                    onClick={() => onRemoveMail(mail)}
                    title="move to the ricycle bin"
                  >
                    {" "}
